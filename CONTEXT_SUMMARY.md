...
- **Discussion équipe / animateur** : bouton « Écrire à l'animateur » dans l'exercice (fil partagé par toute l'équipe), carte « Messages des équipes » dans la console (non lus, son, message à toutes les équipes). Messages stockés dans `state.json` (200 max par équipe, 500 caractères, 1,5 s minimum entre deux messages d'une équipe).
- Minuteur par défaut : 4 h (240 min), ajout libre de minutes depuis la console.
- **Chronologie de l'attaque (corrigée)** : 2 mai accès initial par le mail du faux recruteur (le premier contact), 4-5 mai reconnaissance depuis le poste compromis, à partir du 6 mai exfiltration, 8-9 mai porte dérobée sur la passerelle. Avant correction, la reconnaissance datait du 28-29 avril, donc avant l'accès initial, ce qui contredisait la réponse attendue.
- **Onglet ENQUÊTE** : fiches façon avis FBI (date et lieu de naissance, cheveux, yeux, taille, poids, sexe, nationalité, ancienneté, signes particuliers), cohérentes avec les âges, anciennetés et profils OSINT. Pas de champ « race » (interdit en France et inutile à l'enquête).
...
- **Pièges Vercel Blob rencontrés (septembre 2026)** : (1) dès que `state.json` dépasse quelques Ko, la lecture le sert compressé avec un ETag faible `W/"..."` que l'écriture conditionnelle refuse : il faut retirer le préfixe `W/` (sinon toutes les écritures échouent en « Precondition failed: ETag mismatch ») ; (2) Blob renvoie aussi « conflicting operation » (à traiter comme un conflit et rejouer) ; (3) plusieurs instances de fonctions : chaque écriture incrémente `state.rev`, les pages envoient la dernière version vue (`minRev`) pour ne jamais recevoir un état plus ancien que leurs propres actions. Les écritures d'une instance passent en file d'attente. Fonctions en région `cdg1` (Paris, comme le store) via `vercel.json`.
- Tests sur l'infrastructure réelle sans toucher la production : `vercel deploy -e STATE_PATH=preview-state.json -e ANIM_PASSWORD=...` (fichier d'état isolé), accès à la preview protégée via l'en-tête `x-vercel-protection-bypass`.
...
## Groupes TD créés par l'animateur (26 septembre 2026)

- Les élèves ne créent plus rien. `/salle` : ils appuient sur leur TD, puis leur groupe, puis tapent nom et prénom. Groupe complet grisé.
